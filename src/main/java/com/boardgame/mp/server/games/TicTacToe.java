...
public class TicTacToe extends Game {
  ...
  public ResponseEntity<Object> move(Player player, String object) {
    ...
    if (grid[move.getX()][move.getY()] == null) {
      ...
      if (message.equals(GameResponse.OK)) update(arrayToString());
  ...
  private GameResponse put(int y, int x, Player player) {
    if (player.equals(player1) || player.equals(player2)) {
      ...
      if (!turn.equals(player)) {
        ...
        grid[x][y] = turn;
        return GameResponse.OK;
      } else throw new WrongTurnException("Error : Wrong TicTacToe player turn");
    }
  ...
  public Player checkWinner() {
    ...
    */
    ...
    for (int j = 0; j < grid.length - 1; j++) {
      ...
      if (j + 1 == grid.length - 1) return grid[0][2];
      x--;
    }
